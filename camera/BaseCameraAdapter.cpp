...
void BaseCameraAdapter::enableMsgType(int32_t msgs, frame_callback callback, event_callback eventCb, void* cookie)
{
    ...
    if ( CameraFrame::PREVIEW_FRAME_SYNC == msgs )
        ...
        {
        CAMHAL_ALOGEA("Message type subscription no supported yet!");
        }
...
void BaseCameraAdapter::disableMsgType(int32_t msgs, void* cookie)
{
    ...
    if ( CameraFrame::PREVIEW_FRAME_SYNC == msgs )
        ...
        {
        CAMHAL_ALOGEB("Message type 0x%x subscription no supported yet!", msgs);
        }
  ...
  if ((frameBuf != NULL) && ( pBuf != NULL) )
    {
      ...
      CAMHAL_ALOGVB("Adding Frame=0x%x Y=0x%x UV=0x%x", frame->mBuffer, frame->mYuv[0], frame->mYuv[1]);
    }
}
...
void BaseCameraAdapter::removeFramePointers()
{
  ...
  CAMHAL_ALOGVB("Removing %d Frames = ", size);
  for (int i = 0; i < size; i++)
    {
      CameraFrame *frame = (CameraFrame *)mFrameQueue.valueAt(i);
      CAMHAL_ALOGVB("Free Frame=0x%x Y=0x%x UV=0x%x", frame->mBuffer, frame->mYuv[0], frame->mYuv[1]);
      delete frame;
    }
  mFrameQueue.clear();
}
...
void BaseCameraAdapter::returnFrame(void* frameBuf, CameraFrame::FrameType frameType)
{
    ...
    if ( NULL == frameBuf )
        {
        CAMHAL_ALOGEA("Invalid frameBuf");
        return;
        }
    ...
    if ( NO_ERROR == res)
        ...
        if ( 0 < refCount )
            ...
            {
            CAMHAL_ALOGEA("Frame returned when ref count is already zero!!");
            return;
            }
        }

    //CAMHAL_ALOGVB("REFCOUNT 0x%x %d", frameBuf, refCount);
...
status_t BaseCameraAdapter::sendCommand(CameraCommands operation, int value1, int value2, int value3)
{
    ...
    switch ( operation ) {
        case CameraAdapter::CAMERA_USE_BUFFERS_PREVIEW:
                ...
                if ( NULL == desc )
                    {
                    CAMHAL_ALOGEA("Invalid preview buffers!");
                    return -EINVAL;
                    }
        ...
        case CameraAdapter::CAMERA_USE_BUFFERS_PREVIEW_DATA:
                    ...
                    if ( NULL == desc )
                        {
                        CAMHAL_ALOGEA("Invalid preview data buffers!");
                        return -EINVAL;
                        }
        ...
        case CameraAdapter::CAMERA_USE_BUFFERS_IMAGE_CAPTURE:
                ...
                if ( NULL == desc )
                    {
                    CAMHAL_ALOGEA("Invalid capture buffers!");
                    return -EINVAL;
                    }
...
// ymjun temp : If we use "#ifdef ICAP", snapshot function with sub camera does not work. 
        ...
        default:
            CAMHAL_ALOGEB("Command 0x%x unsupported!", operation);
            break;
    };
...
status_t BaseCameraAdapter::notifyShutterSubscribers()
{
    ...
    if ( mShutterSubscribers.size() == 0 )
        {
        CAMHAL_ALOGEA("No shutter Subscribers!");
        return NO_INIT;
        }
    ...
    for (unsigned int i = 0 ; i < mShutterSubscribers.size() ; i++ ) {
        ...
        CAMHAL_ALOGEA("Sending shutter callback");
...
status_t BaseCameraAdapter::sendFrameToSubscribers(CameraFrame *frame)
{
    ...
    if ( NULL == frame )
        {
        CAMHAL_ALOGEA("Invalid CameraFrame");
        return -EINVAL;
        }
    ...
    for( mask = 1; mask < CameraFrame::ALL_FRAMES; mask <<= 1){
      if( mask & frame->mFrameMask ){
        ...
        case CameraFrame::PREVIEW_FRAME_SYNC:
          {
            if ( frame_skip < 4 ) {  // To fix blinking issue (0814)
                frame_skip++;
                for ( int i=getFrameRefCount(frame->mBuffer, CameraFrame::PREVIEW_FRAME_SYNC); i>0; --i ) {
                    returnFrame(frame->mBuffer, CameraFrame::PREVIEW_FRAME_SYNC);
                }
            } else {
        ...
        case CameraFrame::FRAME_DATA_SYNC:
          ...
          break;
        default:
          CAMHAL_ALOGEB("FRAMETYPE NOT SUPPORTED 0x%x", mask);
        break;
        }//SWITCH
                                                     ...
                                                     CameraFrame::FrameType frameType)
{
    ...
    if ( (frameType == CameraFrame::PREVIEW_FRAME_SYNC) ||
        ...
        if (mFrameQueue.size() > 0){
          ...
          CAMHAL_ALOGEA("Empty Frame Queue");
          return -EINVAL;
        }
      }
    ...
    if (NULL != subscribers) {
        ...
        if (refCount > subscribers->size()) {
            CAMHAL_ALOGEB("Invalid ref count for frame type: 0x%x", frameType);
            return -EINVAL;
        }

        //CAMHAL_ALOGVB("Type of Frame: 0x%x address: 0x%x refCount start %d",
        //             frame->mFrameType,
        ...
        for ( unsigned int i = 0 ; i < refCount; i++ ) {
            ...
            if (!callback) {
                CAMHAL_ALOGEB("callback not set for frame type: 0x%x", frameType);
                return -EINVAL;
            }
        ...
        CAMHAL_ALOGEA("Subscribers is null??");
        return -EINVAL;
    }
  ...
  for( lmask = 1; lmask < CameraFrame::ALL_FRAMES; lmask <<= 1){
    if( lmask & mask ){
      ...
      case CameraFrame::FRAME_DATA_SYNC:
        ...
        break;
      default:
        CAMHAL_ALOGEB("FRAMETYPE NOT SUPPORTED 0x%x", lmask);
        break;
      }//SWITCH
      mask &= ~lmask;
    }//IF
...
status_t BaseCameraAdapter::startPreview(char* indexes)
{
    status_t ret = NO_ERROR;
    ...
    switch ( mAdapterState )
        ...
        case INTIALIZED_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch INTIALIZED_STATE Invalid Op! event = 0x%x",
                                 operation);
        ...
        case PREVIEW_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_SMOOTH_ZOOM:
                    CAMHAL_ALOGDB("Adapter state switch PREVIEW_ACTIVE->PREVIEW_STATE event = 0x%x",
                                 operation);
                    mNextState = PREVIEW_STATE;
                    break;

                default:
                    CAMHAL_ALOGEB("Adapter state switch PREVIEW_ACTIVE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case LOADED_CAPTURE_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch LOADED_CAPTURE_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case CAPTURE_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch CAPTURE_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case BRACKETING_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch BRACKETING_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case AF_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch AF_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case ZOOM_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch ZOOM_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case VIDEO_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch VIDEO_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case VIDEO_AF_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch VIDEO_AF_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case AF_ZOOM_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch AF_ZOOM_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case VIDEO_ZOOM_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch VIDEO_ZOOM_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case BRACKETING_ZOOM_STATE:
            ...
            switch ( operation )
                ...
                default:
                    CAMHAL_ALOGEB("Adapter state switch BRACKETING_ZOOM_STATE Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        default:
            CAMHAL_ALOGEA("Invalid Adapter state!");
            ret = INVALID_OPERATION;
        }
