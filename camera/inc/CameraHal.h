...
#define CAMHAL_ALOGVA(str)
#define CAMHAL_ALOGVB(str, ...)

//#define CAMHAL_ALOGEA ALOGE
//#define CAMHAL_ALOGEB ALOGE

#define CAMHAL_ALOGEA(a)
#define CAMHAL_ALOGEB(a, ...)

#else

#define CAMHAL_ALOGDA DBGUTILS_ALOGDA
#define CAMHAL_ALOGDB DBGUTILS_ALOGDB
#define CAMHAL_ALOGVA DBGUTILS_ALOGVA
#define CAMHAL_ALOGVB DBGUTILS_ALOGVB
...
#define USE_AUTOFOCUS_THREAD
#define MMS_COMBINATION_BUG_FIX
//#define USE_CAF_CALLBACK

namespace android {

// 20121018 daewon1004.kim@lge.com add rotation information.
enum
{
       ROTATION_0 =0,
	ROTATION_90,     // main camera  (vt call)
	ROTATION_270   //  sub camer     (vt call)
};
...
//#include "icam_icap/maker_note_mms_public.h"
}
#endif
...
#define VIDEO_DEVICE_INDEX_MAIN       0x0000
#define VIDEO_DEVICE_INDEX_SECONDARY  0x0001
#define VIDEO_DEVICE_INDEX_MASK       0x000F
#define VIDEO_DEVICE_INDEX(x) (x & VIDEO_DEVICE_INDEX_MASK)
...
#ifndef LOG_FUNCTION_NAME	// Remove compiler warning message
...
#endif // #ifndef LOG_FUNCTION_NAME
...
#define IMX046_FOCALLENGTH 3.67
#define IMX046_HORZANGLE 47.11
#define IMX046_VERTANGLE 36.21
#define MIN_FPS 8
#define MAX_FPS 30

#define SNAPSHOT_MAX_HEIGHT            600
...
#define DSP3630_KHZ_MIN 520000 // Kirti Added
...
#define IPP_Missing_Mode -1
#define IPP_Disabled_Mode 0
#define IPP_CromaSupression_Mode 1
#define IPP_EdgeEnhancement_Mode 2
#define CAPTURE_MODE_MISSING          -1
#define CAPTURE_MODE_HIGH_PERFORMANCE 1
#define CAPTURE_MODE_HIGH_QUALITY     2
#define CAPTURE_HP_W 1600
#define CAPTURE_HP_H 1200
...
#define PHOTO_PATH          "/mnt/sdcard/photo_%02d.%s"
...
#define PROC_THREAD_DUMMYCB     0x4
#define PROC_THREAD_PROCESS         0x5
#define PROC_THREAD_EXIT            0x6
#define PROC_THREAD_NUM_ARGS    49
...
#define AFCB_THREAD_CALL        0x1
#define AFCB_THREAD_EXIT        0x2
#define AFCB_THREAD_NUM_ARGS    4
#endif // #ifdef USE_AUTOFOCUS_THREAD
// rt5604 2012.08.16 autoFocus <-


// rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom ->
#define ZOOMCB_THREAD_CALL        0x1
#define ZOOMCB_THREAD_EXIT        0x2
#define ZOOMCB_THREAD_NUM_ARGS    5
// rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom <-
...
typedef struct {
    ...
    /* hold 2A status */
    SICam_Status   status;
    SICam_Status   status_before_af;
    SICam_Status   status_icap;
    bool have_status_before_af;
...
/* CSR-OMAPS00275734 kirti.badkundri@sasken.com fix Number of Buffer mismatch in CamerHal.cpp and AppCallbackNotifier.cpp [START] */    
    static const int32_t MAX_BUFFERS = 6;  // 8 -> 6
/* CSR-OMAPS00275734 kirti.badkundri@sasken.com fix Number of Buffer mismatch in CamerHal.cpp and AppCallbackNotifier.cpp [END] */    
...
public:
    ...
    virtual char * GetFrame(int &index, int rotateVideo) = 0;
...
public:
    ...
    char buffers_in_display[MAX_BUFFERS+1];
};
...
/*--------------------Eclair HAL---------------------------------------*/
   ...
   // rt5604 2012.08.16 autoFocus ->
#ifdef USE_AUTOFOCUS_THREAD
   class AutoFocusCBThread : public Thread {
	   CameraHal* mHardware;
   public:
	   AutoFocusCBThread(CameraHal* hw)
		   : Thread(false), mHardware(hw) { }
   
	   virtual bool threadLoop() {
		   mHardware->autoFocusCBThread();
		   return false;
	   }
   };
#endif // #ifdef USE_AUTOFOCUS_THREAD
   ...
   // rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom <-
   class ZoomCBThread : public Thread {
	   CameraHal* mHardware;
   public:
	   ZoomCBThread(CameraHal* hw)
		   : Thread(false), mHardware(hw) { }
   
	   virtual bool threadLoop() {
		   mHardware->zoomCBThread();
		   return false;
	   }
   };
   // rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom <-
   ...
   CameraHal(int cameraId);
    ...
    void snapshotThread();
#ifdef USE_AUTOFOCUS_THREAD
    void autoFocusCBThread();	// rt5604 2012.08.16 autoFocus
#endif // #ifdef USE_AUTOFOCUS_THREAD

    void zoomCBThread();			// rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom 
   ...
   bool mSetPreviewWindowCalled;
...
#endif // #ifdef USE_AUTOFOCUS_THREAD
	void onZoom(bool cb_value, int zoom_value);		// rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom
    ...
    int FW3A_Stop_AF();
    int FW3A_Reset_AF();
    bool FW3A_Reset_AF_enabled() const;
    int FW3A_GetSettings() const;
    int FW3A_SetSettings();
    int FW3A_MakeIcapStatus();
    int FW3A_Set_TouchFocus(int x, int y,
	...
	// 20120628 jungyeal@lge.com sub cam rotation patch from mms
	// 20121018 daewon1004.kim@lge.com change to int form boolean
    char * GetFrame(int &index, int rotateVideo);
    ...
    void JpegEncoderCreate(void);
    void JpegEncoderDestroy(void);
    ...
    void setParameterSupportedSizeList(CameraParameters &params, const char *key, const char *delimiter, const supported_resolution *supRes, size_t count);
    void setParameterSupportedRangeList(CameraParameters &params, const char *key, const char *delimiter, const supported_range *supRange, size_t count);
    void setParameterSupportedRangeListToSingle(CameraParameters &params, const char *key, const char *delimiter, const supported_range *supRange, size_t count);
    int getParameterPreviewFpsRange(const CameraParameters &params, int *min_fps, int *max_fps) const;
    status_t setParametersSC(bool reset);
...
/* 20120628 jungyeal@lge.com sub cam rotation patch from mms [END] */
    ...
    int isStart_FW3A;
    int isStart_FW3A_AF;
    int isStart_FW3A_AF_Reset;
    int isStart_FW3A_CAF;
    int isStart_FW3A_AEWB;
    int isStart_VPP;
    int FW3A_AF_TimeOut;
    bool isSet_TouchFocus;
...
#ifdef USE_AUTOFOCUS_THREAD
	sp<AutoFocusCBThread> mAutoFocusCBThread;	// rt5604 2012.08.16 autoFocus
#endif // #ifdef USE_AUTOFOCUS_THREAD
	sp<ZoomCBThread> mZoomCBThread;		// rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom 
	int mTotalJpegsize;
	void* mJpegBuffAddr;
    ...
    bool mDisplayPaused;
    bool mIPPToEnable;
    bool mPreviewChangedNeedsRestart;
    ...
    sp<MemoryHeapBase> mYUV420SP_Heap;
    sp<MemoryBase> mYUV420SP_MemBase;
/* 20120628 jungyeal@lge.com sub cam rotation patch from mms [START] */
// 20121003 daewon1004.kim@lge.com Front camera display -90 on the vt call (blackg_open_ics)
    ...
    int mCameraWidth;
    int mCameraHeight;
    sp<MemoryHeapBase> mCameraHeaps[MAX_CAMERA_BUFFERS];
    sp<MemoryBase> mCameraBuffers[MAX_CAMERA_BUFFERS];
    bool mFaceUnlockMode;
    bool mvt_mode;	
#endif 
/* 20120628 jungyeal@lge.com sub cam rotation patch from mms [END] */
...
#define debugShowBufferStatus()
    ...
    int mCameraIndex;
    //Bitmap ID of current camera adapter
    int mCameraID;
    ...
    int mfirstTime;
    static const char *camera_device_name[MAX_CAMERAS_SUPPORTED];
    static const int idxPreviewRes[MAX_CAMERAS_SUPPORTED];
    static const supported_resolution supportedPreviewRes[];
    static const int idxThumbnailRes[MAX_CAMERAS_SUPPORTED];
    static const supported_resolution supportedThumbnailRes[];
    static const int idxPictureRes[MAX_CAMERAS_SUPPORTED];
    static const supported_resolution supportedPictureRes[];
    static const int idxFpsRange[MAX_CAMERAS_SUPPORTED];
    static const supported_range supportedFpsRange[];
...
#ifdef USE_AUTOFOCUS_THREAD
    int afcbPipe[2];			// rt5604 2012.08.16 autoFocus
#endif // #ifdef USE_AUTOFOCUS_THREAD
    int zoomcbPipe[2];			// rt5604 2012.08.15 Bug Fix for CTS testSmoothZoom
    int mippMode;
    int pictureNumber;
    ...
    int lastOverlayBufferDQ;
        ...
        int CameraDeviceSetControl(const char *ids, int request, int value);
...
/*--------------Eclair Camera HAL---------------------*/
   ...
   JpegEncoder*    jpegEncoder;
    ...
    int mSCConfigOld_brightness;
    int mSCConfigOld_white_balance;
    int mSCConfigOld_color_effect;
    int mSCConfigOld_anti_banding;
    int mSCConfigOld_metering_mode;
    int mSCConfigOld_night_mode;

    Vector< sp<CameraArea> > mFocusAreas;

    int mLastFocusMode;
    int caf_type;
    int caf_type_changed;
