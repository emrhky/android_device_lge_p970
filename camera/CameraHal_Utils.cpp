...
int CameraHal::FW3A_Release()
{
    ...
    ret = ICam_Release(fobj->hnd, ICAM_RELEASE);
    if (ret < 0) {
...
int CameraHal::FW3A_Start()
{
    ...
    if (VIDEO_DEVICE_INDEX(mCameraIndex) != VIDEO_DEVICE_INDEX_MAIN) {
        ALOGI("Fake 3A FW start on non primary video device");
        return 0;
    }
...
int CameraHal::FW3A_Stop()
{
    ...
    if (VIDEO_DEVICE_INDEX(mCameraIndex) != VIDEO_DEVICE_INDEX_MAIN) {
        ALOGI("Fake 3A FW stop on non primary video device");
        return 0;
    }
...
int CameraHal::FW3A_MakeIcapStatus()
{
    int ret = 0;
    bool icap_stat_overwrite = false;

    {
        Mutex::Autolock lock(mParametersLock);
        if ( mParameters.get(CameraParameters::KEY_FOCUS_MODE) != NULL ) {
            if (strcmp(mParameters.get(CameraParameters::KEY_FOCUS_MODE), (const char *) CameraParameters::FOCUS_MODE_AUTO ) == 0) {

                icap_stat_overwrite = true;

            } else if (strcmp(mParameters.get(CameraParameters::KEY_FOCUS_MODE), (const char *) CameraParameters::FOCUS_MODE_MACRO) == 0) {
    ...
    memcpy(&fobj->status_icap, &fobj->status, sizeof(SICam_Status));

    if (icap_stat_overwrite && fobj->have_status_before_af) {
        ALOGD("FW3A_MakeIcapStatus: overwrite AWB and AE status with those taken before AF");
        memcpy(&fobj->status_icap.ae, &fobj->status_before_af.ae, sizeof(SICam_StatusAE));
        memcpy(&fobj->status_icap.awb, &fobj->status_before_af.awb, sizeof(SICam_StatusAWB));
        memcpy(&fobj->status_icap.preview, &fobj->status_before_af.preview, sizeof(SICam_Rect));
    }

    return ret;
}

int CameraHal::FW3A_Set_TouchFocus(int x, int y, int w, int h)
{
    int err = 0;
    ...
    if ((0 >= w) || (0 >= h)) {
        ALOGE("FW3A_Set_TouchFocus: Non positive dimensions (%d)x(%d)", w, h);
        err = -1;
    }
    else {
        ...
        if ((0 > x) || (0 > y)) {
            ALOGE("FW3A_Set_TouchFocus: Stop touch focus (%d),(%d)", x, y);
            ...
            ALOGE("FW3A_Set_TouchFocus: Stop touch focus because focus coordinates (%d),(%d) out of range (%d),(%d)",
                 x, y, w-1, h-1);
            ...
            icam_ft.enable = 1;
            icam_ft.count = 1;
            icam_ft.faces[0].top = y;
            icam_ft.faces[0].left = x;
            icam_ft.faces[0].width = 0;
            icam_ft.faces[0].height = 0;
        ...
        err = ICam_FaceTracking(fobj->hnd, &icam_ft);
        if (err) {
            ALOGE("FW3A_Set_TouchFocus: ICam_FaceTracking error %d", err);
        } else {
            CAMHAL_ALOGDB("left = %d; top = %d", x, y);
        }
    }
...
// 20121003 daewon1004.kim@lge.com Front camera display -90 on the vt call (blackg_open_ics)
    ...
    //if ((VIDEO_DEVICE_INDEX(mCameraIndex) == VIDEO_DEVICE_INDEX_SECONDARY)  &&  (mRotateVideo == true))
    ALOGI("kim vt  1 CorrectPreview mRotateVideo:%d, w:%d, h: %d, mCameraWidth:%d, mCameraHeight:%d",
                         mRotateVideo, dst_width, dst_height, mCameraWidth, mCameraHeight);
    if ( mRotateVideo)		
    {
        int tmp;
        tmp = dst_width;
...
int CameraHal::CameraDeviceSetControl(const char *ids, int request, int value) {
    int ret;
    struct v4l2_control v_control;

    ALOGI("CameraDeviceSetControl: %s 0x%08x 0x%08x", ids, request, value);

    v_control.id = request;
    v_control.value = value;

    ret = ioctl(camera_device, VIDIOC_S_CTRL, &v_control);

#ifndef DEBUG_LOG
    if ( ret < 0) {
        ALOGE("%s Failed (%d): %s", ids, ret, strerror(errno));
    }else{
        ALOGD("%s Succeed %d", ids, ret);
    }
#endif
