...
{   
    ...
    return 2;//num_cameras_supported;
}
...
status_t CameraProperties::loadProperties()
{
    ...
    if((int)mCamerasSupported < 0) {
        ...
        for (unsigned int i = 0; i < mCamerasSupported; i++) {
            ...
            if ( i == 0 ) {
                mCameraProps[i].set(ORIENTATION_INDEX, CAMERA0_ORIENTATION);
            }
            else if ( i == 1 ) {
                mCameraProps[i].set(ORIENTATION_INDEX, CAMERA1_ORIENTATION);
            }
            mCameraProps[i].set(FACING_INDEX, i);
