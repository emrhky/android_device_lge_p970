...
void* MemoryManager::allocateBuffer(int width, int height, const char* format, int &bytes, int numBufs)
{
    ...
    if(!bufsArr)
        {
        CAMHAL_ALOGEB("Allocation failed when creating buffers array of %d uint32_t elements", numArrayEntriesC);
    ...
    if(bytes != 0)
        ...
        for (int i = 0; i < numBufs; i++)
            ...
            if(ret < 0)
                {
                CAMHAL_ALOGEB("ion_alloc resulted in error %d", ret);
                goto error;
                }

            CAMHAL_ALOGDB("Before mapping, handle = %x, nSize = %d", handle, bytes);
            if ((ret = ion_map(mIonFd, handle, bytes, PROT_READ | PROT_WRITE, MAP_SHARED, 0,
                          (unsigned char**)&bufsArr[i], &mmap_fd)) < 0)
                {
                CAMHAL_ALOGEB("Userspace mapping of ION buffers returned error %d", ret);
...
int MemoryManager::freeBuffer(void* buf)
{
    ...
    if(!bufEntry)
        {
        CAMHAL_ALOGEA("NULL pointer passed to freebuffer");
        LOG_FUNCTION_NAME_EXIT;
        return BAD_VALUE;
        }
    ...
    while(*bufEntry)
        ...
        if(mIonBufLength.valueFor(ptr))
            ...
            {
            CAMHAL_ALOGEA("Not a valid Memory Manager buffer");
            }
        }
