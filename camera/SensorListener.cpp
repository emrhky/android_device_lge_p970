    ...
    while ((num_sensors = listener->mSensorEventQueue->read(sen_events, 8)) > 0) {
        for (int i = 0; i < num_sensors; i++) {
            if (sen_events[i].type == Sensor::TYPE_ACCELEROMETER) {
                ...
                CAMHAL_ALOGVA("ACCELEROMETER EVENT");
                CAMHAL_ALOGVB(" azimuth = %f pitch = %f roll = %f",
                ...
                CAMHAL_ALOGVB(" tilt = %d orientation = %d", tilt, orient);
            } else if (sen_events[i].type == Sensor::TYPE_GYROSCOPE) {
                CAMHAL_ALOGVA("GYROSCOPE EVENT");
            }
        }
    }

    if (num_sensors < 0 && num_sensors != -EAGAIN) {
        CAMHAL_ALOGEB("reading events failed: %s", strerror(-num_sensors));
    }
...
SensorListener::~SensorListener() {
    ...
    CAMHAL_ALOGDA("Kill looper thread");
    ...
    CAMHAL_ALOGDA("Kill looper");
    if (mLooper.get()) {
        mLooper->removeFd(mSensorEventQueue->getFd());
        mLooper.clear();
        mLooper = NULL;
    }
    CAMHAL_ALOGDA("SensorListener destroyed");
...
status_t SensorListener::initialize() {
    ...
    if (mSensorEventQueue == NULL) {
        CAMHAL_ALOGEA("createEventQueue returned NULL");
        ret = NO_INIT;
        goto out;
    }
    ...
    if (mSensorLooperThread.get() == NULL) {
        CAMHAL_ALOGEA("Couldn't create sensor looper thread");
        ret = NO_MEMORY;
        goto out;
    }
    ...
    if (ret == INVALID_OPERATION){
        CAMHAL_ALOGDA("thread already running ?!?");
    } else if (ret != NO_ERROR) {
        CAMHAL_ALOGEA("couldn't run thread");
        goto out;
    }
...
void SensorListener::enableSensor(sensor_type_t type) {
    ...
    if ((type & SENSOR_ORIENTATION) && !(sensorsEnabled & SENSOR_ORIENTATION)) {
        ...
        CAMHAL_ALOGDB("orientation = %p (%s)", sensor, sensor->getName().string());
